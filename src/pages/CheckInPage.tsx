...
export function CheckInPage() {
  ...
  const fetchList = async (movieName: string, showTime: string) => {
    ...
    try {
      const [{ data: pData, error: pErr }, { data: cData, error: cErr }] = await Promise.all([
        supabase.from('participants').select('*').eq('movie_name', movieName).eq('show_time', showTime),
        supabase.from('checked_in_participants').select('*').eq('movie_name', movieName).eq('show_time', showTime)
      ]);
  ...
  useEffect(() => {
    if (step === 'movie') fetchMovies();
      ...
      const sub = supabase.channel('sync').on('postgres_changes', { event: '*', schema: 'public' }, () => fetchList(selectedMovie.name, selectedShow.show_time)).subscribe();
  ...
  const handleCheckIn = async (participant: Participant) => {
    ...
    try {
      ...
      const { error: insErr } = await supabase.from('checked_in_participants').insert({ ...cleanInfo, checked_in_at: new Date().toISOString() });
  ...
  const handleUndoCheckIn = async (participant: Participant) => {
    ...
    try {
      ...
      if (insErr) throw insErr;
      await supabase.from('checked_in_participants').delete().eq('id', participant.id);
  ...
  return (
    <div className="min-h-screen bg-[#f9f0e7]/30">
      {toast && (
        <div className={cn('fixed top-5 right-5 z-50 flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold shadow-xl animate-slide-up', toast.ok ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white')}>
      ...
      <header className="sticky top-0 z-40 border-b border-[#33130d]/10 bg-[#f9f0e7]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-4">
            <button onClick={() => { if (step === 'movie') window.history.back(); else if (step === 'show') setStep('movie'); else setStep('show'); }} className="p-2 hover:bg-[#33130d]/5 rounded-lg transition-colors">
            ...
            <div className="flex flex-col">
              ...
              <h1 className="font-cinematic font-bold text-[#33130d] text-xl mt-1">{step === 'movie' ? 'Select Movie' : step === 'show' ? selectedMovie?.name : 'Entrance List'}</h1>
            </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            {step === 'list' && (
              <div className="text-right hidden sm:block">
                <p className="text-sm font-black text-[#a41e22] leading-none">{checkedParticipants.length} / {participants.length + checkedParticipants.length}</p>
              </div>
            )}
            <Button variant="ghost" size="sm" onClick={() => { if (step === 'movie') fetchMovies(); else if (step === 'show' && selectedMovie) fetchShows(selectedMovie.id); else if (step === 'list' && selectedMovie && selectedShow) fetchList(selectedMovie.name, selectedShow.show_time); }} disabled={loading}>
            ...
            {shows.map((show: Show) => (
              <button key={show.id} onClick={() => { setSelectedShow(show); setStep('list'); }} className="w-full bg-white rounded-2xl p-6 flex items-center justify-between movie-shadow border border-[#33130d]/5 hover:border-[#a41e22]/30 hover:bg-[#a41e22]/5 transition-all group">
                <div className="flex items-center gap-4 text-left">
                  <div className="h-12 w-12 rounded-xl bg-[#f9f0e7] text-[#a41e22] flex items-center justify-center group-hover:bg-[#a41e22] group-hover:text-white transition-colors"><Calendar className="h-6 w-6" /></div>
                  <div>
              ...
              {filteredItems.map((p) => {
                ...
                return (
                  <div key={p.id} className={cn("rounded-2xl p-5 movie-shadow border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-6", isCheckedIn ? "bg-emerald-50 border-emerald-200" : "bg-white border-[#33130d]/5")}>
                    ...
                    <div>
                      {isCheckedIn ? (
                        <button onClick={() => handleUndoCheckIn(p)} className="flex items-center justify-center gap-2 bg-emerald-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-600 transition-all group">
                          <Check className="h-5 w-5 group-hover:hidden" /> <X className="h-5 w-5 hidden group-hover:block" />
                          <span className="group-hover:hidden">Verified</span> <span className="hidden group-hover:block">Undo?</span>
                        </button>
                      ) : (
                        <button onClick={() => handleCheckIn(p)} disabled={!!checkingIn} className={cn("flex items-center justify-center gap-3 bg-[#33130d] text-white px-8 py-4 rounded-xl font-bold hover:bg-[#a41e22] transition-all active:scale-95 shadow-lg shadow-[#33130d]/20", checkingIn === p.id && "bg-[#a41e22] opacity-80")}>
                          {checkingIn === p.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <UserCheck className="h-5 w-5" />} Verify Entry
                        </button>
                      )}
                    </div>
                  </div>
